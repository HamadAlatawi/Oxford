  ...
  const categories = [
    ...
    const { form, enhance, constraints } = superForm(defaults(zod(newContactSchema)), {
      ...
      validators: zod(newContactSchema),
          ...
          onUpdated({ form }) {
            uploadImage(form.data.Name, form.data.price, form.data.sDesc, form.data.lDesc);
        },
    })
...
<div class="pl-2 w-full h-full overflow-hidden">
    ...
    <p class="mb-5">Fill in the fields below to add your product to the DECOMM customer facing website</p>
    <hr>
    <form method="POST" use:enhance>
      <div class="grid w-full max-w-sm items-center gap-1.5 mt-5">
        ...
        <p class="text-sm text-muted-foreground">Enter the name of the desired product you'd like to sell</p>
      </div>
      <div class="grid w-full max-w-sm items-center gap-1.5 mt-10">
        ...
        <p class="text-sm text-muted-foreground">Enter the Category for your product.</p>
    </div>
      <div class="grid w-full max-w-sm items-center gap-1.5 mt-10">
          ...
          <p class="text-sm text-muted-foreground">Enter the desired Currency you'd like to recieve for your sales.</p>
      </div>
      <div class="grid w-full max-w-sm items-center gap-1.5 mt-10">
      ...
      <div class="grid w-full max-w-sm items-center gap-1.5 mt-10">
          ...
          <Input type="text" id="sDesc" name="sDesc" bind:value={$form.sDesc} {...$constraints.sDesc} placeholder="A car jumper to help start your car" />
          <p class="text-sm text-muted-foreground">Enter a brief description of the product</p>
      </div>
      <div class="grid w-full gap-1.5 mt-10">
          <Label for="lDesk">Full Description</Label>
          <Textarea id="lDesc" name="lDesc" bind:value={$form.lDesc} {...$constraints.lDesc}  placeholder="A portable battery device that allows you to jump start your vehicle. These devices operate similar to jumnper cables but do not require an additional vehicle to provide the power needed to boost the dead vehicle battery. Jump starts using lead-acid batteries that have 1000 amp rating." />
